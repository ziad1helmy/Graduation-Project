...
- **Request Body:**
  ...
  ```
- **Response (201 Created):** Returns the created appointment object including a `qrToken` field and a `status` of `pending`.
...
Request body for reschedule must include `appointmentDate` as an ISO 8601 timestamp. Date-only payloads are rejected to avoid accidental midnight bookings.
- **Request Body:**
  ```json
  {
    "appointmentDate": "2026-10-16T11:00:00.000Z",
    "donationType": "blood | plasma | platelets | double red cells"
  }
  ```
...
└── Reschedule Donation Flow
    ...
    └── Confirmation Screen  ───────────────────→ PATCH /donations/book-appointment/:id
                                                   (receives updated qrToken in response)
```
...
- **Generated by the backend** at appointment creation (`POST /donations/book-appointment`) and updated on reschedule (`PATCH /donations/book-appointment/:appointmentId`).
- **Returned as a unique token string** in the `qrToken` field of the appointment object.
