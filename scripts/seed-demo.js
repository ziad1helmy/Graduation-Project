...
async function ensureUser(model, payload) {
  ...
  if (existingBase && existingBase.role !== userPayload.role) {
    throw new Error(`Existing user role mismatch for ${userPayload.email}: expected ${userPayload.role}, found ${existingBase.role}`);
  }
  ...
  if (!existingBase) {
    user = await model.create({
      ...
      emailVerifiedAt: now,
      isSuspended: userPayload.isSuspended || false,
      deletedAt: null,
    });
  } else {
    ...
    user.deletedAt = null;
    await user.save();
    // Reload to get clean state after save
    user = await model.findById(existingBase._id).select('+adminKey');
  }

  // Encrypt adminKey so it stays decryptable (avoids bcrypt pre-save hash)
  if (plaintextKey) {
...
async function main() {
  ...
  requests.cairoCriticalBlood = await ensureRequest(
    { hospitalId: hospitals.cairoCare._id, notes: '[demo-seed] cairo-critical-o-positive' },
    {
      ...
      hospitalContact: hospitals.cairoCare.phone,
  ...
  requests.gizaHighBlood = await ensureRequest(
    { hospitalId: hospitals.nileHope._id, notes: '[demo-seed] giza-high-a-negative' },
    {
      ...
      hospitalContact: hospitals.nileHope.phone,
